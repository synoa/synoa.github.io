...
**Note:** Your message will most likely be inside the Spam folder since "cmd@localhost" is most likely flagged as spam (at
least in GMail).
...
You'll maybe get an error saying `AH01797: client denied by server configuration` in `/var/log/apache2/error.log`, to
prevent this add the following to `apache2.conf`.

```xml
 <Directory "/var/www/public_html/project/">
  AllowOverride all
  Order allow,deny
  Allow from all
</Directory>
